...
import {
  enrichFilms,
  enrichFilmPosterFromTMDB,
} from "./services/filmEnrichmentService";
...
function App() {
  ...
  const [currentScrapingUsername, setCurrentScrapingUsername] = useState<
    string | undefined
  >(undefined);
  const [followingFeatureEnabled, setFollowingFeatureEnabled] = useState<
    boolean | null
  ...
  const handleSubmit = async (usernamesArray: string[]) => {
    ...
    try {
      ...
      console.log(
        `[${timestamp}] [COMPARE] Starting comparison for ${usernamesArray.length} users: ${usernamesArray.join(", ")}`,
      );
      ...
      for (let i = 0; i < usernamesArray.length; i++) {
        ...
        try {
          ...
          if (!films) {
            console.log(
              `[${new Date().toISOString()}] [CACHE] Cache miss for ${username} watchlist, fetching...`,
            );
            films = await fetchWatchlist(username);
            sessionCache.setWatchlist(username, films);
          } else {
            console.log(
              `[${new Date().toISOString()}] [CACHE] Cache hit for ${username} watchlist (${films.length} films)`,
            );
          }

          if (!profile) {
            console.log(
              `[${new Date().toISOString()}] [CACHE] Cache miss for ${username} profile, fetching...`,
            );
            profile = await fetchUserProfile(username);
            sessionCache.setProfile(username, profile);
          } else {
            console.log(
              `[${new Date().toISOString()}] [CACHE] Cache hit for ${username} profile`,
            );
          }
          ...
          if (i > 0 && userFilms.length >= 2) {
            ...
            const commonFilms = findCommonFilmsAcrossUsers(
              previousFilms,
              currentFilms,
            );
            ...
            if (commonFilms.length > 0) {
              console.log(
                `[${new Date().toISOString()}] [ENRICH] Found ${commonFilms.length} common films, ${enrichmentQueue.length} queued for enrichment`,
              );
            }
          }
        } catch (err) {
          ...
          console.error(
            `[${new Date().toISOString()}] [ERROR] Failed to fetch data for ${username}:`,
            errorMessage,
          );
          console.error(
            `[${new Date().toISOString()}] [ERROR] Full error details for ${username}:`,
            errorDetails,
          );
          console.error(
            `[${new Date().toISOString()}] [ERROR] Error object for ${username}:`,
            err,
          );
      ...
      if (failedUsers.length > 0 && validUsers.length >= 2) {
        console.warn(
          `[${new Date().toISOString()}] [WARNING] Some users failed but continuing with ${validUsers.length} valid users. Failed: ${failedUsers.join(", ")}`,
        );
      }
      ...
      console.error(
        `[${new Date().toISOString()}] [ERROR] Comparison failed:`,
        err,
      );
      setIsLoading(false);
      setCurrentScrapingUsername(undefined);
  ...
  const enrichCommonFilmsInBackground = async (
    ...
    if (films.length === 0) {
      console.log(
        `[${new Date().toISOString()}] [ENRICH] No common films need enrichment`,
      );
      return;
    }

    console.log(
      `[${new Date().toISOString()}] [ENRICH] Starting background enrichment for ${films.length} common films...`,
    );
    ...
    for (let i = 0; i < films.length; i++) {
      ...
      if (enriched && enriched.posterUrl) {
        console.log(
          `[${new Date().toISOString()}] [ENRICH] Using cached poster for "${film.title}" (${i + 1}/${films.length})`,
        );
      } else {
        ...
        console.log(
          `[${new Date().toISOString()}] [ENRICH] Fetching poster for "${film.title}" from TMDB (${i + 1}/${films.length})...`,
        );
      ...
      if (enriched && enriched.posterUrl) {
          ...
          return {
            ...
            userGroups: prevResult.userGroups.map((group) => ({
              ...
              commonFilms: group.commonFilms.map((f) => {
                ...
                return fKey === filmKey
                  ? { ...f, posterUrl: enriched!.posterUrl }
                  : f;
              }),
            })),
          };
        ...
        console.log(
          `[${new Date().toISOString()}] [ENRICH] ✓ Poster loaded for "${film.title}" (${i + 1}/${films.length})`,
        );
      } else {
        console.log(
          `[${new Date().toISOString()}] [ENRICH] ✗ No poster found for "${film.title}" (${i + 1}/${films.length})`,
        );
      }
    }

    console.log(
      `[${new Date().toISOString()}] [ENRICH] Background enrichment completed`,
    );
  };
    ...
    <div className="app">
      <header className="app-header">
        <div className="header-content">
          ...
          />
          <h1>Letterbuds</h1>
          <a href="/style-test" className="header-style-test-link">
            Style test
          </a>
        </div>
      </header>
      ...
      <main className="app-main">
        {!isLoading && !result && !error && (
          <p className="intro-text">
            Compare Letterboxd watchlists to find movies you all want to watch
          </p>
        )}
        ...
        {isLoading && (
          <LoadingSpinner currentUsername={currentScrapingUsername} />
        )}
